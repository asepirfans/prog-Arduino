...
void setup() {
 ...
 WiFi.mode(WIFI_STA);
  ...
  // Try to connect to saved WiFi credentials or start the WiFiManager portal
  bool res = wifiManager.autoConnect("Tomato WiFiManager", "password");
  if (!res) {
    Serial.println("Failed to connect");
    previousMillis = millis(); // Start the timer if not connected
  } else {
...
void loop() {
  ...
  if (WiFi.isConnected()) {
    ...
    runProgram(); // Call the main program function
  } else {
    Serial.println("WiFi Not Connected");
    if (currentMillis - previousMillis >= interval) {
      Serial.println("No WiFi connection for 10 seconds. Resetting WiFi settings...");
  ...
  delay(500); // Delay for 1 second for the next status check
}
